...
export function AuthScreen() {
  ...
  const [mode, setMode] = useState<"signin" | "activate" | "reset">("signin");
  ...
  const go = (m: "signin" | "activate" | "reset") => {
    setMode(m);
    setError(null);
    setSent(false);
  };
    ...
    <div className="min-h-full bg-white flex flex-col px-6 pt-16 pb-10">
      <motion.div initial={{ opacity: 0, y: -16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }}>
        ...
        <h1 className="font-display text-[32px] leading-tight tracking-tight text-[var(--bq-text-primary)] mb-2">
          {mode === "signin" ? "Welcome back" : mode === "reset" ? "Reset password" : "Set up your account"}
        </h1>
        <p className="text-[var(--bq-text-secondary)]">
          {mode === "signin" ? `Sign in to ${appName}` : mode === "reset" ? "We'll email you a link to set a new password." : "First time? Create your password to get started."}
        </p>
      </motion.div>

      {mode === "reset" && sent ? (
        <div className="mt-10">
          <div className="text-sm rounded-[0.9rem] px-4 py-3" style={{ color: "#067647", background: "#ecfdf3" }}>
            If an account exists for {email.trim()}, a reset link is on its way. Open it on this device to set a new password.
          </div>
          <button onClick={() => go("signin")} className="mt-6 text-[var(--bq-text-secondary)] text-sm mx-auto block">
            Back to sign in
          </button>
        </div>
      ) : (
        <>
          ...
          <button
            onClick={() => go(mode === "signin" ? "activate" : "signin")}
            className="mt-6 text-[var(--bq-text-secondary)] text-sm mx-auto"
          >
            {mode === "signin" ? "First time here? Set up your account" : mode === "reset" ? "Back to sign in" : "Already have an account? Sign in"}
          </button>
        </>
      )}
    </div>
